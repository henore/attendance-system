...
router.post('/break/end', async (req, res) => {
  try {
    ...
    // 実際の現在時刻で終了時刻を設定
    const breakDuration = timeToMinutes(currentTime) - timeToMinutes(attendance.break_start);
  ...
  router.get('/monthly-attendance', async (req, res) => {
    try {
      ...
      const targetUser = await dbGet(
        'SELECT id, name, role, service_type, service_no, transportation FROM users WHERE id = ? AND is_active = 1',
        [userId]
      );
